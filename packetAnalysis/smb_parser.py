...
def extract_smb_data(smb_packets):
    ...
    for packet in smb_packets:
        ...
        smb_command = parse_smb2_header(raw_data)
        if smb_command is None:
            continue
        
        print(f"SMB command: {smb_command}")  # Print SMB command for debugging

        if smb_command == 9:  # SMB2 Write Request
            smb_write_request = parse_smb2_write_request(raw_data)
            if smb_write_request:
                print(f"Write request: {smb_write_request}")  # Print write request details for debugging
                metadata = {
                    "src_ip": ip_layer.src,
        ...
        elif smb_command == 10:  # SMB2 Write Response
            smb_write_response = parse_smb2_write_response(raw_data)
            if smb_write_response:
                print(f"Write response: {smb_write_response}")  # Print write response details for debugging
                metadata = {
                    "src_ip": ip_layer.src,
                    "src_port": tcp_layer.sport,
                    "dst_ip": ip_layer.dst,
